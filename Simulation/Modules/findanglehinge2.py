...
left=-1
initialthetay=0
initialthetaz=0#np.deg2rad(-0.08783658335221914)
...
deflectionlistZ2,x=deflectionpointload(Z2,0.0,x2,ntotal,right, initialthetaz,Iyyrotated()) #goes through start of wall, causes no deflection?
deflectionlistZ3,xnegativedirection=deflectionpointload(Z3,x3-x2,la-x2,ntotal,left, -initialthetaz,Iyyrotated())
deflectionlistP,xnegativedirection=deflectionpointload(P,xa/2.,la-x2,ntotal,left, -initialthetaz,Iyyrotated())
deflectionlistR,x= deflectionpointload(R,xa/2.,x2,ntotal,right, initialthetaz,Iyyrotated())

#y
deflectionlistY1,x=deflectionpointload(Y1,x2-x1,x2,ntotal,right, initialthetay,Izzrotated())
deflectionlistY2,x=deflectionpointload(Y2,0.0,x2,ntotal,right, initialthetay,Izzrotated()) #goes through start of wall, causes no deflection?
deflectionlistY3,xnegativedirection=deflectionpointload(Y3,x3-x2,la-x2,ntotal,left, -initialthetay,Izzrotated())
deflectionlistq1,x=deflectiondistributedload(q,x2,ntotal,right, initialthetay,Izzrotated())
deflectionlistq2,xnegativedirection=deflectiondistributedload(q,la-x2,ntotal,left, -initialthetay,Izzrotated())
