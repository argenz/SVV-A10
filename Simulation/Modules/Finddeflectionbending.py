...
def deformduetobending():
    ...
    deflectionlistzthetaright=np.array(x)*(thetaz)
    ...
    #merging the 2 lists
    zdeflections=np.concatenate((dzleft[::-1],dzright))
    ...
    deflectionlistythetaright=np.array(x)*(thetay)
    ...
    #merging the 2 lists
    ydeflections=np.concatenate((dyleft[::-1],dyright))

    #############
    #Merge x coordinates list
    xcoordinateslist=np.concatenate((xnegativedirection[::-1],x))
    ...
    return xcoordinateslist,ydeflections,zdeflections,x1discr,x3discr
