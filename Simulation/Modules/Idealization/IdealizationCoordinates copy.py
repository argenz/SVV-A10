...
def IdealizationCoordinates():
    C_a = 0.547
    h_a = 0.225
    n_st = 17 #total number of stringers
    n_p = 6 #total number of stringers on top or bottom panel
    ...
    B_coordinatesTP.pop() #Removes last elemt that would coincide with that of the arc
    
    #STRINGER COORDINATES FOR LEADING EDGE ARC top half------------
    B_coordinatesA = list()
    B_coordinatesA.append([0,h_a/2])
    B_coordinatesA.append([m.sin(45*(m.pi/180))*(h_a/2),m.cos(45*(m.pi/180))*(h_a/2)])
    B_coordinatesA.append([h_a/2,0])
...
t111, t222, x = IdealizationCoordinates()